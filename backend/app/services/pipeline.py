"""
Processing Pipeline Orchestrator (Updated for real ISRO data)

Chains: Ingestion → Preprocessing → Matching → Verification → Geospatial Mapping
"""
...
from app.models.schemas import (
    JobStatus, ResultsResponse, ImageInfo, MatchResult,
)
...
def get_matcher() -> FeatureMatcher:
    """Singleton FeatureMatcher — loaded once, reused across jobs."""
    global _matcher
    if _matcher is None:
...
class ProcessingPipeline:
    ...
    async def run(
        ...
        pixel_a, meta_a, geom_a = ingest_image(path_a)
        pixel_b, meta_b, geom_b = ingest_image(path_b)
        ...
        report(JobStatus.PREPROCESSING, 20, "Preprocessing",
               f"Processing {meta_a.instrument.value.upper()} image...")
        patches_a, processed_a = preprocess_image(pixel_a, meta_a)

        report(JobStatus.PREPROCESSING, 30, "Preprocessing",
               f"Processing {meta_b.instrument.value.upper()} image...")
        ...
        report(JobStatus.MATCHING, 65, "Feature Matching",
        ...
        report(JobStatus.VERIFYING, 80, "Verification",
               f"Verified {len(kpts_a)} matches (from {total_raw} raw)")

        # ── Step 6: Geospatial Mapping ──
        report(JobStatus.MAPPING, 85, "Geospatial Mapping",
               "Converting to lunar coordinates...")

        match_points, confidence_score, geo_stats = map_matches_to_coordinates(
            kpts_a, kpts_b, confidence,
            meta_a, meta_b, verify_stats,
            geometry_a=geom_a,
            geometry_b=geom_b,
        )
        ...
        result = ResultsResponse(
            ...
            image_a=ImageInfo(
                ...
                bbox={
                    "lat_min": meta_a.bbox_lat_min, "lat_max": meta_a.bbox_lat_max,
                    "lon_min": meta_a.bbox_lon_min, "lon_max": meta_a.bbox_lon_max,
                },
            ),
            image_b=ImageInfo(
                ...
                bbox={
                    "lat_min": meta_b.bbox_lat_min, "lat_max": meta_b.bbox_lat_max,
                    "lon_min": meta_b.bbox_lon_min, "lon_max": meta_b.bbox_lon_max,
                },
            ),
