...
const FishyFileDrop: React.FC<FishyFileDropProps> = ({
  ...
  backgroundImage = "https://cdn.21st.dev/assets/mirror/e4/e4c5ffc34c0135331e34f6473097748e7d8ca5f7453e73696082129b8f7811f0.png",
  ...
  const borderGradientLayerStyle: React.CSSProperties = {
    position: "absolute",
    inset: 0,
  ...
  const bubbleStyle = (left?: string, right?: string, bottom?: string, size?: string, color?: string): React.CSSProperties => ({
    position: "absolute",
    zIndex: 7,
    width: size,
    height: size,
    ...
    bottom,
    background: color || (typeof bubbleColor === "string" ? bubbleColor : "rgba(255,255,255,0.8)"),
    borderRadius: "50%",
  });
  ...
  const waveElements = waveAnimations.map((wave, i) => {
    ...
    const waveBottomStyle: React.CSSProperties = {
      ...waveTopStyle,
      top: "auto",
      left: "auto",
      bottom: "-20px",
  ...
  const bubbleElements = bubblePositions.map((bubble, i) => (
    <motion.div
      key={`bubble-${i}`}
      style={bubbleStyle(bubble.left, (bubble as { right?: string }).right, bubble.bottom, bubble.size, bubbleColorList[i])}
